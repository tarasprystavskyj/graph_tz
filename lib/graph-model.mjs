...
export const graphFiles = {
  ...
  "server-telemetry": {
    ...
    path: resolve(
      ...
      "happyuser-visual_link_analisis-c4307a2aa7f8",
      "sample_graphs",
      "server_telemetry_hub_tz_cytoscape.json",
    ),
  },
  "current-project-learning": {
    label: "Current project learning graph",
    path: resolve(repoRoot, "graph-ui", "public", "data", "current_project_learning_cytoscape.json"),
  },
};
...
export async function readGraphPayload(graphId = "job-apply-full") {
  ...
  return JSON.parse(await readFile(graph.path, "utf8"));
}
...
async function applyGraphEdits(model, graphId) {
  const editsPath = resolve(repoRoot, "graph-ui", "data", "graph-edits", `${graphId}.json`);
  let edits;
  try {
